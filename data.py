...
class Data:
    ...
    grammar_files_json = "docs/grammar/"
    ...
    def get_public_fp(self, fp:str):
        ...
        if self.check_corpus_type(fp) == 'sentence':
            ...
            return fp.replace(".txt", ".json")
        if self.check_corpus_type(fp) == 'grammar':
            fp = fp.replace(self.grammar_files_raw, self.grammar_files_json)
            return fp.replace(".txt", ".json")
